...
step :: Grid -> Grid;
step = reset_flashed . resolve_flashes . inc_all
...
steps_until :: (Grid -> Bool) -> Grid -> Int;
steps_until p g = length (takeWhile (not . p) (iterate step g))

all_flashed :: Grid -> Bool;
all_flashed = all (== 0)

main :: IO ();
main = let { g = make_grid input1 } in
         putStrLn (show (steps_until all_flashed g))
